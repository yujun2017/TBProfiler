...
class bam:
    ...
    def call_variants(self,ref_file,caller,bed_file=None,threads=4):
        ...
        if bed_file:
            self.windows_cmd = "cat %(bed_file)s | awk '{print $1\":\"$2\"-\"$3\" %(prefix)s\"$1\"_\"$2\"_\"$3}'" % vars(self)
        else:
            self.windows_cmd = "bedtools makewindows -g %(ref_file)s.fai -n %(threads)s | awk '{print $1\":\"$2+1\"-\"$3\" %(prefix)s\"$1\"_\"$2+1\"_\"$3}'" % vars(self)
